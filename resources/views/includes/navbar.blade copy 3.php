...
<header x-data="{ openMenu: false, scrolled: false }" @scroll.window="scrolled = (window.pageYOffset > 20)"
    ...
    <div class="w-full max-w-[1400px] mx-auto flex items-center justify-between px-2 lg:px-6">
        ...
        <div class="flex items-center lg:w-1/4">
            <a href="{{ url('/') }}" class="flex items-center gap-2 sm:gap-3 shrink-0 group">
                <div class="relative flex items-center justify-center">
                    <div
                        class="absolute -inset-1 bg-indigo-500 rounded-full blur opacity-0 group-hover:opacity-20 transition duration-500">
                    </div>
                    <img src="{{ asset('images/logo.png') }}" alt="AR Logo"
                        class="h-10 lg:h-12 w-auto object-contain relative z-10">
                </div>
                <div class="flex items-center pb-2 lg:pt-1.5">
                    <span
                        class="whitespace-nowrap text-xl xl:text-3xl text-indigo-600 tracking-tight group-hover:text-black-800 transition-colors duration-300 font-bold">
                        ENGINEERING
                    </span>
                </div>
            </a>
        ...
        <nav class="hidden lg:flex items-center justify-center space-x-1 xl:space-x-6 flex-grow">
            ...
            <a href="{{ route('home') }}" class="relative px-2 py-2 text-sm xl:text-base font-bold transition-colors duration-200 group
               {{ request()->routeIs('home') ? 'text-indigo-600' : 'text-slate-700 hover:text-indigo-600' }}">
                Home
                <span
                    class="absolute bottom-0 left-0 w-full h-[2px] bg-indigo-600 transform origin-left transition-transform duration-300
            ...
            <div class="relative group" x-data="{ open: false }" @mouseenter="open = true" @mouseleave="open = false">
                <button
                    class="relative px-2 py-2 text-sm xl:text-base font-bold flex items-center gap-1 transition-colors duration-200
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div
                        class="bg-slate-900 shadow-2xl shadow-slate-900/40 rounded-xl border border-slate-800 overflow-hidden py-2 text-slate-300">

                        <a href="{{ route('sIndex') }}"
                            class="block px-5 py-2.5 text-sm font-semibold hover:bg-slate-800 hover:text-white transition-colors border-l-2 border-transparent hover:border-indigo-500">
                            All Services
                        </a>
                        ...
                        @foreach($mainServices as $service)
                            <a href="{{ route('services.show', $service->slug) }}"
                                class="block px-5 py-2.5 text-sm font-medium hover:bg-slate-800 hover:text-white transition-colors border-l-2 border-transparent hover:border-indigo-500">
                                {{ $service->title }}
                            </a>
                        @endforeach
                    </div>
            ...
            <a href="{{ route('pIndex') }}" class="relative px-2 py-2 text-sm xl:text-base font-bold transition-colors duration-200 group
            ...
            <a href="{{ route('client') }}" class="relative px-2 py-2 text-sm xl:text-base font-bold transition-colors duration-200 group
            ...
            <a href="{{ route('gallary.video') }}" class="relative px-2 py-2 text-sm xl:text-base font-bold transition-colors duration-200 group
        ...
        <div class="hidden lg:flex items-center justify-end lg:w-1/4">
            <a href="{{ $profile }}" target="_blank" download
                ...
                <div class="absolute inset-0 w-full h-full bg-gradient-to-r from-indigo-600 to-indigo-500 opacity-90">
                </div>
                <div
                    class="absolute bottom-0 right-0 block w-64 h-64 mb-32 mr-4 transition duration-500 origin-bottom-left transform rotate-45 translate-x-24 bg-white opacity-10 group-hover:rotate-90">
                </div>
                <span
                    class="relative flex items-center gap-2 text-lg group-hover:font-extrabold group-hover:scale-[1.02] transition-transform duration-300 tracking-wide">
        ...
        <div class="lg:hidden flex items-center z-50">
            <button @click="openMenu = !openMenu"
                ...
                <div
                    class="absolute inset-0 bg-slate-100 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
                </div>
                <svg x-show="!openMenu" class="w-7 h-7 relative z-10" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
    ...
    <template x-teleport="body">
        <div class="lg:hidden relative z-[100]">
            ...
            <div x-show="openMenu" x-transition:enter="transition-opacity ease-linear duration-300"
                ...
                x-transition:leave-end="opacity-0" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-40"
                ...
                activeMenu: '{{ 
                    ...
                    (request()->is('about*') || request()->routeIs('contact.index') || request()->routeIs('howItWorks') ? 'about' : null)) 
                        ...
                        {{-- <div
                            ...
                            --}}
                            {{-- <div
                                class="fixed top-0 right-0 w-1/2 h-full bg-white shadow-2xl z-50 overflow-y-auto flex flex-col border-l border-slate-200">
                            ...
                            <!-- Header -->
                            <div
                                class="flex flex-row items-center justify-between px-3 sm:px-6 py-5 border-b border-slate-100 bg-slate-50/50">
                                <span
                                    class="text-base sm:text-xl font-black text-slate-800 tracking-tight shrink-0">MENU</span>
                                <button @click="openMenu = false"
                                    class="p-1.5 sm:p-2 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 rounded-lg transition-colors shrink-0">
                   ...
                   {{ request()->routeIs('home') ? 'bg-indigo-50 border-indigo-600 text-indigo-700' : 'border-transparent text-slate-700 hover:bg-slate-50 hover:text-indigo-600 hover:border-slate-300' }}">
                            ...
                            {{ request()->is('services*') ? 'bg-indigo-50 border-indigo-600 text-indigo-700' : 'border-transparent text-slate-700 hover:bg-slate-50 hover:text-indigo-600 hover:border-slate-300' }}">
                                        ...
                                        <svg class="w-5 h-5 transform transition-transform duration-300"
                                            :class="{ 'rotate-180': activeMenu === 'services' }" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </button>
                                    <div x-show="activeMenu === 'services'" x-collapse
                                        class="pl-8 pr-4 py-2 space-y-1 bg-slate-50/50">
                                        <a @click="openMenu = false" href="{{ route('sIndex') }}"
                                            class="block py-2 text-sm font-semibold transition-colors {{ request()->routeIs('sIndex') ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                                            All Services
                                        </a>
                                        @foreach($mainServices as $service)
                                            <a @click="openMenu = false" href="{{ route('services.show', $service->slug) }}"
                                                class="block py-2 text-sm font-semibold transition-colors {{ request()->is('services/' . $service->slug) ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                                                {{ $service->title }}
                                            </a>
                                        @endforeach
                                    </div>
                   ...
                   {{ request()->routeIs('client') ? 'bg-indigo-50 border-indigo-600 text-indigo-700' : 'border-transparent text-slate-700 hover:bg-slate-50 hover:text-indigo-600 hover:border-slate-300' }}">
                            ...
                            {{ request()->is('careers*') ? 'bg-indigo-50 border-indigo-600 text-indigo-700' : 'border-transparent text-slate-700 hover:bg-slate-50 hover:text-indigo-600 hover:border-slate-300' }}">
                                        Join Us
                                        <svg class="w-5 h-5 transform transition-transform duration-300"
                                            :class="{ 'rotate-180': activeMenu === 'careers' }" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </button>
                                    <div x-show="activeMenu === 'careers'" x-collapse
                                        class="pl-8 pr-4 py-2 space-y-1 bg-slate-50/50">
                                        <a @click="openMenu = false" href="{{ route('careers.why') }}"
                                            class="block py-2 text-sm font-semibold transition-colors {{ request()->routeIs('careers.why') ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                                            Why Join Us
                                        </a>
                                        <a @click="openMenu = false" href="{{ route('careers.job') }}"
                                            class="block py-2 text-sm font-semibold transition-colors {{ request()->routeIs('careers.job') ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                                            Job Vacancy
                                        </a>
                                        <a @click="openMenu = false" href="{{ route('careers.internship') }}"
                                            class="block py-2 text-sm font-semibold transition-colors {{ request()->routeIs('careers.internship') ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                                            Internship
                                        </a>
                                    </div>
                                </div>
                   ...
                   {{ request()->routeIs('gallary.video') ? 'bg-indigo-50 border-indigo-600 text-indigo-700' : 'border-transparent text-slate-700 hover:bg-slate-50 hover:text-indigo-600 hover:border-slate-300' }}">
                            ...
                            {{ (request()->is('about*') || request()->routeIs('contact.index') || request()->routeIs('howItWorks')) ? 'bg-indigo-50 border-indigo-600 text-indigo-700' : 'border-transparent text-slate-700 hover:bg-slate-50 hover:text-indigo-600 hover:border-slate-300' }}">
                                        About Us
                                        <svg class="w-5 h-5 transform transition-transform duration-300"
                                            :class="{ 'rotate-180': activeMenu === 'about' }" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M19 9l-7 7-7-7" />
                                        </svg>
                                    </button>
                                    <div x-show="activeMenu === 'about'" x-collapse
                                        class="pl-8 pr-4 py-2 space-y-1 bg-slate-50/50">
                                        <a @click="openMenu = false" href="{{ route('about') }}"
                                            class="block py-2 text-sm font-semibold transition-colors {{ request()->routeIs('about') ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                                            About Us
                                        </a>
                                        <a @click="openMenu = false" href="{{ route('contact.index') }}"
                                            class="block py-2 text-sm font-semibold transition-colors {{ request()->routeIs('contact.index') ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                                            Contact
                                        </a>
                                        <a @click="openMenu = false" href="{{ route('howItWorks') }}"
                                            class="block py-2 text-sm font-semibold transition-colors {{ request()->routeIs('howItWorks') ? 'text-indigo-600' : 'text-slate-600 hover:text-indigo-600' }}">
                                            How We Work
                                        </a>
                                    </div>
                                </div>
